...
class MyResnet(nn.Module):
    def __init__(self, materials_dim):
        ...
        materials_linearblock = self.linearblock(num_features + 1, 1024)
        self.materials_decoder = nn.Sequential(*materials_linearblock)
        del materials_linearblock

        self.materials_fc = nn.Linear(1024, materials_dim)

    def linearblock(self, indim, outdim):
        block = [nn.Linear(indim, outdim), nn.ReLU()]
        return block

    def forward(self, x, azimuth):
        x = self.resnet(x)
        ...
        if x.shape[0] == 1:
            ...
            x = x.squeeze()
        x = torch.cat([x, azimuth], dim=1)  # 画像からの特徴量にカメラの情報も入れる

        materials = x
        for layer in self.materials_decoder:
            materials = layer(materials)
        materials = self.materials_fc(materials)
        materials = torch.tanh(materials)  #[-1,1]
        materials = (materials + 1.0) / 2.0  #[0,1]
        return materials
...
class GCN(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        support = torch.bmm(
            input,
            self.weight.unsqueeze(0).expand(input.shape[0], -1, -1))

        output = torch.bmm(
            adj.unsqueeze(0).expand(input.shape[0], -1, -1), support)
