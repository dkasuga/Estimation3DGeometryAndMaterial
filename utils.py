...
def get_pooling_index(positions, cam_mat, cam_pos, dims):
    ...
    for dim in dims:
        ...
        y1s = y1s + torch.arange(positions.shape[0]).float().to(
            positions.device) * dim
        y2s = y2s + torch.arange(positions.shape[0]).float().to(
            positions.device) * dim
...
def pooling(blocks, pooling_indices):
    ...
    for i_block, block in enumerate(blocks):
        ...
        features = (A * C * G + H * D * A + G * E * B + B * F * H).permute(
            1, 0)
...
def chamfer_normal(pred_mesh, gt_points, gt_norms):
    ...
    gt_indices = norm_distance(pred_mesh.vertices.unsqueeze(0),
                               gt_points.unsqueeze(0))[0]
    ...
    vertex_neighbors = pred_mesh.vertices[neighbor_indecies].view(
        new_dimensions)
    ...
    normal_loss = (torch.abs(torch.sum(vertex_norms * vertex_neighbors,
                                       dim=1)))
...
def setup_meshes(filename='meshes/156.obj', device="cuda"):
    ...
    mesh_1.vertices = mesh_1.vertices.to(device)
    mesh_1.faces = mesh_1.faces.to(device)
    # mesh.uvs = mesh.uvs.to(device)
    mesh_1.face_textures = mesh_1.face_textures.to(device)
    # mesh.textures = mesh.textures.to(device)
    mesh_1.edges = mesh_1.edges.to(device)
    mesh_1.vv = mesh_1.vv.to(device)
    # mesh_1.vv_count = mesh_1.vv_count.to(device)
    # mesh_1.vf = mesh_1.vf.to(device)
    # mesh_1.vf_count = mesh_1.vf_count.to(device)
    # mesh_1.ve = mesh_1.ve.to(device)
    # mesh_1.ve_count = mesh_1.ve_count.to(device)
    # mesh_1.ff = mesh_1.ff.to(device)
    # mesh_1.ff_count = mesh_1.ff_count.to(device)
    # mesh_1.ef = mesh_1.ef.to(device)
    # mesh_1.ef_count = mesh_1.ef_count.to(device)
    # mesh_1.ee = mesh_1.ee.to(device)
    # mesh_1.ee_count = mesh_1.ee_count.to(device)

    adj_1 = mesh_1.compute_adjacency_matrix_full().clone()
    adj_1 = normalize_adj(adj_1)
    mesh_1_i = kal.rep.TriangleMesh.from_tensors(mesh_1.vertices.clone(),
                                                 mesh_1.faces.clone())

    mesh_2, split_mx_1 = split_mesh(mesh_1)
    adj_2 = mesh_2.compute_adjacency_matrix_full().clone()
    adj_2 = normalize_adj(adj_2)
    mesh_2_i = kal.rep.TriangleMesh.from_tensors(mesh_2.vertices.clone(),
                                                 mesh_2.faces.clone())

    mesh_3, split_mx_2 = split_mesh(mesh_2)
    adj_3 = mesh_3.compute_adjacency_matrix_full().clone()
    adj_3 = normalize_adj(adj_3)
    mesh_3_i = kal.rep.TriangleMesh.from_tensors(mesh_3.vertices.clone(),
                                                 mesh_3.faces.clone())
...
def split(meshes, features, index):
    meshes['init'][index + 1].vertices = split_features(
        meshes['split_mxs'][index], meshes['update'][index].vertices)
...
def loss_surf(meshes, tgt_points):
    loss = kal.metrics.point.chamfer_distance(meshes['update'][0].vertices,
                                              tgt_points,
                                              w1=1.,
                                              w2=0.55)
    loss += kal.metrics.point.chamfer_distance(meshes['update'][1].vertices,
                                               tgt_points,
                                               w1=1.,
                                               w2=0.55)
    loss += kal.metrics.point.chamfer_distance(meshes['update'][2].vertices,
                                               tgt_points,
                                               w1=1.,
                                               w2=0.55)
    return loss
...
def loss_lap(meshes):
    loss = .1 * kal.metrics.mesh.laplacian_loss(meshes['init'][0],
                                                meshes['update'][0])
    loss += kal.metrics.mesh.laplacian_loss(meshes['init'][1],
                                            meshes['update'][1])
    loss += kal.metrics.mesh.laplacian_loss(meshes['init'][2],
                                            meshes['update'][2])

    loss += torch.sum(
        (meshes['init'][0].vertices - meshes['update'][0].vertices)**2,
        1).mean() * .0666 * .1
    loss += torch.sum(
        (meshes['init'][1].vertices - meshes['update'][1].vertices)**2,
        1).mean() * .0666
    loss += torch.sum(
        (meshes['init'][2].vertices - meshes['update'][2].vertices)**2,
        1).mean() * .0666
