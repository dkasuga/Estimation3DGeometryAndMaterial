...
parser.add_argument(
    ...
    type=str,
    default="/home/mil/kasuga/kaolin_1217/kaolin/examples/ImageRecon/Pixel2Mesh_pyredner/log/0124jinbei_car_specular_sample_roughrange/model/material_estimator_epoch51.pth",
    help='optimizer_path')
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for i in range(args.val_batchsize):
                ...
                out_diffuse_reflectance_batch = inp_diffuse_reflectance_batch
                out_specular_reflectance_batch = out_materials_batch[:, 0:3]
                ...
                writer = imageio.get_writer(os.path.join(args.logdir, 'valid/rotation_epoch{}_{}.gif'.format(self.cur_epoch, i)), mode='i')
                for num, azimuth in enumerate(loop):
                    # rest mesh to initial state
                    loop.set_description('Drawing rotation')
                    azimuth_batch = torch.ones([data[0].shape[0], ], dtype=torch.float32) * azimuth
                    out_scene_list = make_scenes(
                        meshes['update'][2].vertices.unsqueeze(0),
                        meshes['update'][2].faces.unsqueeze(0), azimuth_batch,
                        out_diffuse_reflectance_batch,
